...
def get_stats_analysis(path, n_class, n_output, thres=0.5, n_folds=4, class_=1, test_set=False):
...
def convert_pred(array, thres, n_output):
    ...
    """
    if n_output==2:
        if array[1]>=thres and array[0]>=thres:
            return 1
        elif  array[1]<thres and array[0]>=thres:
            return 0
        else:
