...
def wilcoxon_validation(pairs):
    ...
    if len(orig) != len(trans) or len(orig) < 10:  # Changed from 10 to 5 for consistency
...
def filter_and_transform_molecules(input_file, smiles_label_dict):
    ...
    for line in lines[1:]:
        ...
        if len(parts) >= 10:
            try:
                ...
                if len(parts) > 11:  # e.g. parts[10]: original, parts[11]: transformed
                    try:
    ...
    if p_value is not None and p_value < 0.05:
        ...
        # If not significant, you might want to handle this case differently
        print("No significant transformations found based on Wilcoxon test.")
        filter_transformations = []  # or positive_label_avg_rows if you want to keep them anyway
...
for transform in positive_trans:
    ...
    if not discard_rule:
        ...
        positive_transformed_smiles.append(new_mols)
positive_output_file = "data/positive_transformed_smiles.txt"
positive_output_rule = "data/positive_transformed.txt"
save_transformed_smiles(positive_transformed_smiles, positive_output_file)
save_transformed_smiles(positive_transformed_rule, positive_output_rule)

print("Positive label_avg rules saved to: positive_label_avg_output.txt")
print("Positive transformed SMILES saved to:", positive_output_file)